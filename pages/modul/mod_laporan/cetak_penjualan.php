...
$query= "SELECT *  FROM penjualan, produk where penjualan.id_produk=produk.id_produk ORDER BY penjualan.no_penjualan DESC";
...
if (!empty($query))
	...
	$pdf->Image("images/auto_2000.jpg",2,1.15,'L');
	$pdf->SetFont('Arial','B',14);
	$pdf->Ln();
...
while ($hasil=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(2,1,$no,1,0,'C');
	$pdf->Cell(5,1,$hasil['no_penjualan'],1,0,'C');
	$pdf->Cell(5,1,$hasil['tgl_penjualan'],1,0,'C');
	$pdf->Cell(5,1,$hasil['nama_produk'],1,0,'C');
	$pdf->Cell(5,1,$hasil['jml_penjualan'],1,0,'C');
	$pdf->Cell(5,1,$hasil['petugas'],1,0,'C');
