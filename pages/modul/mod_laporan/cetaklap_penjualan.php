...
$sampai=$_POST[sampai];
  ...
  FROM
	...
	INNER JOIN `produk` ON `produk`.`id_produk` = `penjualan`.`id_produk`							  
			where tgl_penjualan Between '$dari' and '$sampai' ORDER BY id_penjualan DESC";
...
if (!empty($query))
	...
	$pdf->Cell(8,1,'Nomor Penjualan',1,0,'C');
	$pdf->Cell(5,1,'Tanggal Penjualan',1,0,'C');
	$pdf->Cell(8,1,'Nama Produk',1,0,'C');
	$pdf->Cell(4,1,'Jumlah Penjualan',1,0,'C');
	$pdf->Cell(4,1,'Total Penjualan',1,0,'C');
...
while ($hasil=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(2,1,$no.'.',1,0,'C');
	$pdf->Cell(8,1,$hasil['id_penjualan'],1,0,'L');
	$pdf->Cell(5,1,$hasil['tgl_penjualan'],1,0,'L');
	$pdf->Cell(8,1,$hasil['nama_produk'],1,0,'L');
	$pdf->Cell(4,1,$hasil['jml_penjualan'],1,0,'C');
	$pdf->Cell(4,1,$hasil['tot_jual'],1,0,'C');
